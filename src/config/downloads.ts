...
export const EDGE_DOWNLOAD = {
   ...
   * 三平台均 0.3.3（2026-07-10：本机构建；GitHub Actions artifact
   * 上传仍受仓库配额阻断，改走本地构建后上传 ECS）。
   */
  items: [
    { key: 'mac-arm64', label: 'macOS · Apple 芯片（M 系列）', file: 'AIDCP-0.3.3-arm64.dmg' },
    { key: 'mac-x64', label: 'macOS · Intel', file: 'AIDCP-0.3.3.dmg' },
    { key: 'win-x64', label: 'Windows · x64', file: 'AIDCP Setup 0.3.3.exe' },
  ],
} as const;
