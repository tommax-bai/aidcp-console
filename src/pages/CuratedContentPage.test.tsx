 ...
 * 断言：按钮禁用态（评论行两动作全禁、壳行禁参照创作）、两端点调用参数（按行账号路由 + withGroup）、
...
describe('CuratedContentPage 行级定向动作（change curated-note-actions）', () => {
  beforeEach(() => {
    state.items = [
      ...
      makeRow({ id: 9, title: '壳行笔记', body: '' }), // 壳行 → 禁参照创作、可定向评论
    ];
    vi.mocked(apiPost).mockReset();
  });

  it('按钮禁用态：评论行两动作全禁；壳行禁参照创作但可定向评论', async () => {
    renderPage();
    await screen.findByText('目标笔记标题');
    const createBtns = screen.getAllByRole('button', { name: '参照创作' });
    const commentBtns = screen.getAllByRole('button', { name: '定向评论' });
  ...
  it('参照创作：确认后按行账号 POST create-post；triggered=true → 引导去飞书人审', async () => {
    vi.mocked(apiPost).mockResolvedValue({ triggered: true });
    renderPage();
    await screen.findByText('目标笔记标题');
    fireEvent.click(screen.getAllByRole('button', { name: '参照创作' })[0]);
    fireEvent.click(await screen.findByText('触发创作'));
    expect(await screen.findByText(/已触发参照创作/)).toBeTruthy();
    expect(vi.mocked(apiPost)).toHaveBeenCalledWith('/api/curated/contents/7/create-post', { accountId: 'acc-1' });
  });

  it('参照创作域内拒绝（publish_busy）→ 中文原因、绝不染绿', async () => {
    ...
    expect(screen.queryByText(/已触发参照创作/)).toBeNull();
  });

  it('定向评论：弹窗选「带群评论」→ POST comment 带 withGroup:true；未配群码拒绝呈现中文', async () => {
    vi.mocked(apiPost).mockResolvedValue({ triggered: false, reason: 'group_code_missing' });
    renderPage();
    await screen.findByText('目标笔记标题');
    fireEvent.click(screen.getAllByRole('button', { name: '定向评论' })[0]);
  ...
  it('定向评论触发成功 → 提示人审与结果卡、弹窗关闭', async () => {
    vi.mocked(apiPost).mockResolvedValue({ triggered: true });
    renderPage();
    await screen.findByText('目标笔记标题');
    fireEvent.click(screen.getAllByRole('button', { name: '定向评论' })[0]);
    await screen.findByText(/目标笔记：/);
    fireEvent.click(screen.getByRole('button', { name: /触\s*发\s*评\s*论|触发评论/ }));
    expect(await screen.findByText(/已触发定向评论/)).toBeTruthy();
    expect(vi.mocked(apiPost)).toHaveBeenCalledWith('/api/curated/contents/7/comment', {
      accountId: 'acc-1',
      withGroup: false,
    });
