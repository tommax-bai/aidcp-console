  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Alert
        ...
        showIcon
        message="活跃时段与内容自动化"
        description="全局周历是所有账号的默认值；账号表可单独添加排期，账号自定义优先，恢复全局后立即重新继承。每张周历都管两层：绿格=账号活跃（允许浏览会话）；绿格里的白点=该小时还允许自动发内容。常规内容动作可选「关 / 开 / 免审」，平台专用动作按 Cloud 声明展示自己的开关、上限和时段；休眠格绝不自动（云端强制）。手动 /publish、/comment 不受时段限制。点=「允许自动尝试」，非保证发出（日上限、风控、目标范围、身份与页面核对仍会拦）。保存后的账号配置直接生效，不需要运维再开隐藏开关。"
      />
